...
export function CareerPathSection() {
  ...
  useEffect(() => {
    ...
    fetch("https://dev-api.instient.ai/api/career-paths?populate=*", requestOptions)
    ...
    <div className="flex flex-wrap justify-center sm:justify-start gap-28 mt-16 sm:mt-0">
      {careerData.map((item) => (
        <div key={item.id} className="relative mb-14 sm:mb-14 w-full sm:w-[407px]">
          ...
          <Image
            src={`https://dev-api.instient.ai${item.Image.url}`} // Assuming item has BackgroundImage property
