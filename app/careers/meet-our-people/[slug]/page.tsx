...
export default function MeetOurPeopleSlugPage({
  params,
}: {
  ...
  if (!memberData) {
    return (
      <p className="text-center mt-20">Member not found or incomplete data.</p>
    );
  }
    ...
    <main>
      ...
      <div className="relative w-full h-[590px] sm:h-[450px] flex flex-col lg:flex-row gap-4 items-center justify-center p-6 sm:p-8">
        ...
        <div className="text-white text-center sm:text-left mt-5 sm:mt-12 max-w-lg lg:ml-16 z-20 px-4 sm:px-0">
          ...
          {LinkedinUrl && (
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center rounded-full border-2 border-white shadow-lg hover:shadow-[0_0_20px_5px_rgba(59,130,246,0.5)] hover:bg-blue-500 transition-all duration-300 ease-in-out transform hover:scale-110">
        ...
        <div className="absolute inset-0 bg-[#0042ad] -z-10 h-[600px] sm:h-[450px]" />
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-8 md:px-20 py-12 text-black">
        ...
        <div className="mb-12">
          ...
          <EducationItem
            degree={Education_Degree1}
            college={Education_College1}
          />
          <EducationItem
            degree={Education_Degree2}
            college={Education_College2}
          />
        </div>
      </div>
      ...
      {content.map((block, index) => {
        if (block.type === "paragraph" && block.children) {
          return (
            <p
              key={index}
              className="text-base sm:text-lg font-ubuntu mb-6 leading-relaxed"
            >
